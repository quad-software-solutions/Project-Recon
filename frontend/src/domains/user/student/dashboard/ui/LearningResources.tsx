...
import { Book, Video, FileText, Code, ExternalLink, Loader2 } from 'lucide-react';
import { fetchLearningMaterialsApi, downloadLearningMaterialApi } from '@/src/domains/learning/academics/api/academicApi';
...
export default function LearningResources() {
  ...
  useEffect(() => {
    fetchLearningMaterialsApi()
      .then(setMaterials)
      .catch(() => setError('Unable to load materials. Enroll in a program to access resources.'))
      .finally(() => setLoading(false));
  }, []);

  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-brand-border-light/60">
      <h3 className="font-display font-bold text-slate-900 text-xl mb-2">Learning Resources</h3>
      <p className="text-xs text-slate-500 mb-6">Materials for your enrolled programs (from the academic API).</p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {materials.map((res) => {
            ...
            const Icon = cfg.icon;
            return (
              <div key={res.id} className="group bg-slate-50 border border-slate-200 rounded-2xl p-5 hover:bg-white hover:border-brand-red/30 hover:shadow-md transition-all flex flex-col h-full">
                ...
                <h4 className="font-sans font-bold text-slate-900 text-sm leading-snug mb-1">{res.title}</h4>
                ...
                <div className="mt-auto pt-4 border-t border-slate-200/60 flex items-center gap-3">
                  {res.file_url && (
                    <a href={res.file_url} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-brand-red flex items-center gap-1">
                      Open <ExternalLink className="w-3 h-3" />
                    </a>
                  )}
                  <button type="button" onClick={() => downloadLearningMaterialApi(res.id)}
                    className="text-xs font-bold text-slate-600 hover:text-brand-red">
                    Download
                  </button>
                </div>
              </div>
