...
export function formatApiError(err: unknown): string {
  if (isApiError(err)) {
    if (err.status === 400 || err.status === 422) return err.message || 'Please check your input and try again.';
    ...
    if (err.status === 409) return err.message || 'This conflicts with an existing record.';
    ...
    if (err.message && /timeout|aborted/i.test(err.message)) {
      return 'The request timed out. Please try again.';
    }
    return err.message || 'Unable to complete the request.';
  }
